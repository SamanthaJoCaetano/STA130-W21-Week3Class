...
```{r}
# What type of variable is color? Categorical nominal
# What type of plot do we use to visualize this type of distribution? Barplot
# Let's plot it!
coffee_ratings %>% ggplot(aes(x = color)) +
  geom_bar() +
  labs(x="Colour")
...
coffee_ratings <- coffee_ratings %>%
  mutate(color2 = case_when(color == "Blue-Green" | color == "Bluish-Green" ~ "Blue-Green",
                            color == "Green" ~ "Green",
                            color == "None" | is.na(color) ~ "Unknown"))
...
coffee_ratings %>%
  group_by(color2) %>%
  summarise(n = n(),
            mean_rating = mean(total_cup_points),
            median_rating = median(total_cup_points),
            sd_altitude  = sd(altitude_mean_meters))
...
coffee_ratings %>%
  ...
  summarise(n = n(),
            mean_rating = mean(total_cup_points),
            median_rating = median(total_cup_points),
            sd_altitude  = sd(altitude_mean_meters, na.rm=TRUE))
...
newdata <- coffee_ratings %>%
  arrange(desc(total_cup_points)) %>%
  filter(country_of_origin == "Mexico") %>%
  head(n=5) %>%
  select(total_cup_points, processing_method, owner)
...
coffee_ratings %>%
  ...
  summarise(n=n(), mean_rating = mean(total_cup_points)) %>%
...
coffee_ratings %>%
  group_by(country_of_origin) %>%
  summarise(count=n(), mean(total_cup_points)) %>%
  filter(count >= 10) %>%
  #arrange(desc(mean_rating)) %>%
  head(n=5) 
